/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package controllers;

import daos.AnswerDAO;
import dtos.AnswerDTO;
import daos.QuestionDAO;
import dtos.QuestionDTO;
import java.io.IOException;
import java.sql.Date;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

/**
 *
 * @author nguye
 */
@WebServlet(name = "AddQuestionController", urlPatterns = {"/AddQuestionController"})
public class AddQuestionController extends HttpServlet {

    private static final String ERROR = "error.jsp";
    private static final String SUCCESS = "SearchQuestionController";

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String url = ERROR;
        try {
            String question = request.getParameter("txtQuestion");
            String [] listAnswer = request.getParameterValues("txtAnswer");
            boolean isDeleted=false;
            String subjectId=request.getParameter("txtSubject");
            long now=System.currentTimeMillis();
            Date createdDate=new Date(now);
            QuestionDTO dto=new QuestionDTO(0, Integer.parseInt(subjectId), question, createdDate, isDeleted);
            QuestionDAO dao =new QuestionDAO();
            dao.insert(dto);
            int questionId=dao.getQuestionId();
            if(listAnswer!=null){
                int count=0;
                for (String string : listAnswer) {
                    boolean isRight=false;
                    if(count==0){
                        isRight=true;
                    }
                    count++;
                    AnswerDTO answer=new AnswerDTO(string,1, questionId, isRight);
                    AnswerDAO ansDao=new AnswerDAO();
                    ansDao.insert(answer);
                }
            }
            url=SUCCESS;
        } catch (Exception e) {
            log("Error at AddQuestionController" + e.toString());
        } finally {
             request.getRequestDispatcher(url).forward(request, response);
        }
    }

    // <editor-fold defaultstate="collapsed" desc="HttpServlet methods. Click on the + sign on the left to edit the code.">
    /**
     * Handles the HTTP <code>GET</code> method.
     *
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
     */
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        processRequest(request, response);
    }

    /**
     * Handles the HTTP <code>POST</code> method.
     *
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
     */
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        processRequest(request, response);
    }

    /**
     * Returns a short description of the servlet.
     *
     * @return a String containing servlet description
     */
    @Override
    public String getServletInfo() {
        return "Short description";
    }// </editor-fold>

}
